...
## Mise à jour v9.0 — stratégie définitive recommandée

La stratégie retenue est : **option 1 + option 2 ensemble**.

```txt
Téléphone / PC
  -> Dashboard Render HTTPS
  -> WebSocket permanent sortant
  -> Raspberry branché Ethernet sur la box
  -> DR302 local
  -> RS485 Modbus
  -> INVT / Salmson / Wilo / coffret 4
```

Le Raspberry reste le maître local :

- lecture DR302 en local
- commandes locales
- règles de sécurité locales
- buffer offline SQLite
- logs persistants
- reconnexion automatique au cloud

Render ne contacte toujours pas le DR302 en direct. Aucun port Modbus 502 ne doit être ouvert sur Internet.

### Contrôle après installation

1. Redéployer Render.
2. Redémarrer l'agent Raspberry.
3. Vérifier les logs :
...
```txt
WS CONNECTÉ ✅       = Raspberry connecté au cloud
AGENT OK / MODBUS... = Raspberry connecté mais lecture DR302/Modbus lente ou absente
AGENT ATTENTE        = Raspberry non vu par Render
```

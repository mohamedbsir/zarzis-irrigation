...
# ============ CONFIGURATION ============
APP_VERSION = "2026.05.10-zarzis-http-push-v8.8-terrain-express-final"
...
MODBUS_REGISTERS_VALIDATED = os.environ.get("MODBUS_REGISTERS_VALIDATED", "true").strip().lower() in {"1", "true", "yes", "on"}
...
SALMSON_COMMAND_ENABLED = env_bool("SALMSON_COMMAND_ENABLED", True)
...
def start_authorization_state() -> dict:
    """Etat lisible par le dashboard: configuration sans verrou logiciel de pré-installation par défaut."""
    simulation = EDGE_MODE in SIMULATION_MODES
    connected = client_is_open()
    agent_safety = cache.get("agent_safety", {}) if isinstance(cache.get("agent_safety"), dict) else {}
    agent_reported = bool(agent_safety)
    edge_allow_start = agent_safety.get("edge_allow_start") if agent_reported else None

    software_lock_reasons: list[str] = []
    runtime_reasons: list[str] = []
    if simulation:
        start_allowed = True
    else:
        cloud_ok = bool(MODBUS_REGISTERS_VALIDATED)
        if not cloud_ok:
            software_lock_reasons.append("Render: MODBUS_REGISTERS_VALIDATED=false")

        if EDGE_MODE in HTTP_PUSH_MODES:
            if not connected:
                runtime_reasons.append("Agent local absent ou données trop anciennes")
            if agent_reported and not bool(edge_allow_start):
                software_lock_reasons.append("Agent local: EDGE_ALLOW_START=false")
            if not agent_reported:
                runtime_reasons.append("Agent local: état EDGE_ALLOW_START pas encore remonté")
            agent_ok = connected and (not agent_reported or bool(edge_allow_start))
        else:
            agent_ok = True

        start_allowed = cloud_ok and agent_ok and not runtime_reasons

    reasons = software_lock_reasons + runtime_reasons
    software_lock_active = bool(software_lock_reasons)
    if software_lock_active:
        safe_state = "SOFTWARE_START_LOCKED"
    elif start_allowed:
        safe_state = "START_READY"
    else:
        safe_state = "WAITING_FOR_AGENT_OR_DATA"

    return {
        "start_commands_allowed": bool(start_allowed),
        "software_start_lock_active": software_lock_active,
        "software_lock_reasons": software_lock_reasons,
        "runtime_reasons": runtime_reasons,
        "mode": EDGE_MODE,
        "simulation": simulation,
        "server_safety_enabled": SERVER_SAFETY_ENABLED,
        "modbus_registers_validated": bool(MODBUS_REGISTERS_VALIDATED),
        "edge_allow_start": edge_allow_start,
        "agent_reported": agent_reported,
        "agent_connected_fresh": bool(connected) if EDGE_MODE in HTTP_PUSH_MODES else None,
        "salmson_command_enabled": bool(SALMSON_COMMAND_ENABLED),
        "planning_enabled": bool(ENABLE_PLANNING),
        "reasons": reasons,
        "software_start_config": {
            "render": "MODBUS_REGISTERS_VALIDATED=true",
            "edge_agent": "EDGE_ALLOW_START=true",
            "rollback_render": "MODBUS_REGISTERS_VALIDATED=false pour forcer un blocage logiciel",
            "rollback_edge_agent": "EDGE_ALLOW_START=false pour forcer un blocage logiciel local",
            "salmson": "SALMSON_COMMAND_ENABLED=true dans le pack terrain express",
        },
        "safe_current_state": safe_state,
    }
    ...
    return jsonify(
        {
            ...
            "simulation": EDGE_MODE in SIMULATION_MODES,
            "start_authorization": start_authorization_state(),
        }
    )
        ...
        return jsonify(
            {
                ...
                "storage": {"data_dir": str(DATA_DIR), "persistent": PERSISTENT_STORAGE_ENABLED},
                "start_authorization": start_authorization_state(),
                "agent_safety": cache.get("agent_safety", {}),
                "invt": cache["invt"],
...
def edge_push():
    ...
    with lock:
        ...
        if isinstance(body.get("safety"), dict):
            cache["agent_safety"] = dict(body["safety"])
        cache["connected"] = True
        cache["mode"] = EDGE_MODE
        ...
        snapshot = {
            ...
            "safety": dict(cache.get("agent_safety", {})),
        }
